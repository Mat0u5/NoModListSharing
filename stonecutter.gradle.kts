...
publishMods {
	...
	if (shouldAnnounceDiscord) {
		...
		discord("publishDiscordBody") {
			...
			if (!isDev) {
				content = changelog.map { "# [NoModListSharing version `$version` is out!](https://modrinth.com/mod/nomodlistsharing/versions)\n" +
					"### Changelog:\n" +
					"```\n$it```\n"  }
			}
			else {
				content = changelog.map { "# [NoModListSharing version `$version` is out!](https://modrinth.com/mod/nomodlistsharing-dev/versions)\n" +
					"### Changelog:\n" +
					"```\n$it```\n" }
			}
